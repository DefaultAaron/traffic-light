...
#
# Usage:
#   scripts/training/train_yolov13.sh s                             # YOLOv13-s defaults
...
if [[ -z "${1:-}" ]]; then
    echo "usage: scripts/training/train_yolov13.sh <n|s|m|l> [extra args]" >&2
    exit 1
fi
...
SEED="${SEED:-0}"        # override with: SEED=42 scripts/training/train_yolov13.sh s
...
# with the actual training seed (yolo CLI: `scripts/training/train_yolov13.sh s seed=42`
