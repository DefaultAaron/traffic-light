...
> **状态（2026-05-12）**：**Used for R1 alt-track selection — R1 closed**。YOLOv13-s + DEIM-D-FINE-S/M/L 已完成训练并归档于 [`../../docs/reports/phase_2_round_1_report.md`](../../docs/reports/phase_2_round_1_report.md) §R1 备选架构 / §DEIM-D-FINE 专项评估。本文作为 R1 选型决策来源保留，paper-related content carries forward to R3 / paper draft。
>
...
| 组件 | 路径 | 说明 |
|------|------|------|
| YOLOv13-s 训练脚本 | `scripts/training/train_yolov13.sh` | ⚠️ YOLOv13 自带 `DSC3k2` / `HyperACE` 模块，**无法用 stock Ultralytics 加载**。需单独克隆 [iMoonLab/yolov13](https://github.com/iMoonLab/yolov13) 并创建独立 venv。`configs/yolov13s.yaml` 已禁用（重命名为 `.bak`） |
...
| YOLO→COCO 转换器 | `scripts/dataset/yolo_to_coco.py` | 读 `data/merged/`，写 `data/merged/annotations/instances_{train,val}.json`。**不复制图片**，与 Ultralytics 共用同一份图像目录 |
| DEIM 训练 wrapper | `scripts/training/train_deim.sh` | `./scripts/training/train_deim.sh {s\|m} -t weights/deim_dfine_{s,m}_coco.pth` |
