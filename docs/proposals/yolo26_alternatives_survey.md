...
> **状态（2026-04-22）**：初步调研，用于 R2 结果出炉后的决策参考。当前 R1 主力为 YOLO26s（mAP50-95 = 0.628 @ 640），R2 训练目标为 1280 分辨率、10–14 类联合模型。本文评估是否值得并行训练 1–2 个替代架构作为对照或备选。
>
> **结论先行**：保持 YOLO26 为主力；并行对照采用 **YOLOv13-s**（最低集成风险）与 **DEIM-D-FINE-S/M**（Apache-2.0，精度上限）。放弃 DEIM-D-FINE-N。不更换到 RT-DETR 家族。
>
> **当前进度**：`configs/yolov13s.yaml`、`DEIM/configs/deim_dfine/deim_hgnetv2_{n,s,m}_traffic_light.yml` 已创建；`scripts/yolo_to_coco.py` 与 `scripts/train_deim.sh` 已就绪；`pyproject.toml` 增加 `deim` 可选组。
...
| 组件 | 路径 | 说明 |
|------|------|------|
| YOLOv13-s 配置 | `configs/yolov13s.yaml` | 复用 Ultralytics `main.py train` 入口 |
| DEIM 项目配置 | `DEIM/configs/deim_dfine/deim_hgnetv2_{n,s,m}_traffic_light.yml` | N 档保留但不计划训练 |
...
- **保持 YOLO26 为主力**。R2 @1280 训练仍应按计划跑完
- **可选**：用空闲 GPU 时间并行跑一次 **YOLOv13-s @1280**，作为 R2 对照。集成成本低（~0.5 天），即使结果平平也不亏

### 中期（R2 结果出炉后评估）

- 若 R2 YOLO26s-1280 的黄灯 / 远距离小目标 mAP 仍不达标 → 启动 **DEIM-D-FINE-S** 试点：
  1. 先在 Orin 上完成 `MultiscaleDeformableAttnPlugin_TRT` 的 OSS 构建（~1 天，结果可长期复用）
  2. 在 55k 合并数据集上训练 DEIM-D-FINE-S @1280（Apache-2.0 许可，为商用化铺路）
  3. 与 YOLO26s-R2 同验证集对比

### 长期（5/15 之后）

- DEIM-D-FINE-S 若验证有效，可作为**商用化路径的首选架构**（规避 Ultralytics 企业许可费用）
- R3 如有 JetPack 升级到 6.x / TRT 10+ 的机会，RF-DETR 与 DEIMv2 可重新评估
