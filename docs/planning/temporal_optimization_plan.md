...
- [ ] **d. 决策** — 三选一写入 `runs/_tsm_decisions.json`（**plural**，与 `runs/_kd_decisions.json` / `runs/_r2_precision_decisions.json` 命名一致）；schema `scripts/_tsm_decision_schema.json`（NEW，deferred）；executor `scripts/_tsm_decide_phase.py`（NEW，deferred；与 `_kd_decide_cell.py` / `_r2_decide_precision.py` ≥ 80% 字段重用）。决策分支按以下**完整覆盖**：
    - **deploy**：Phase 1-A 通过 Gate-1A **AND** 激活 tag 对应 bucket recall delta > 0（不允许"OR 条件通过 + 激活 tag bucket 未改善"路径）+ Phase 1-B 通过 Gate-1B + Phase 1-C ONNX/TRT 验证通过（含引擎 sidecar 四字段一致 + Orin 实测延迟 < 26 ms）；
    - **defer (INT8 重评)**：Phase 1-A 通过 + Phase 1-B 或 Phase 1-C Orin 端到端延迟落在 26-35 ms 区间（含 Phase 1-C 因 cache wiring 引入新延迟的情形）—— 触发 [`additional_components_plan.md`](additional_components_plan.md) §十 INT8 QAT pre-SAHI 同款探针逻辑（在同 round 启动 INT8 校准 + export 改造），随后 Phase 1-C 在 INT8 引擎上重测；
    - **defer (Phase 1-C 失败但 1-A/1-B 通过)**：Phase 1-C ONNX export 检测到自定义算子 / trtexec build 失败 / sidecar 字段缺失 / sidecar engine_sha256 与计算值不一致 / 引擎 eval-parity > 0.01 pp —— defer 至 export 改造完成（属于已识别 carry-forward；**不**触发 INT8）；Phase 1-B 通过的权重作为 PT-only 候选保留至 R3 重评；
    - **drop (Gate-1A 失败)**：Phase 1-A 失败（小目标 recall +< 2 pp 且 mAP +< 1 pp）—— §1.5 明确要求**回到 §0.2 重新评估问题归因，禁止 hyperparam iteration**；
    - **drop (Gate-1B 精度失败)**：Phase 1-A 通过但 Phase 1-B 小目标桶 recall < 0.6（即 §0.2 启动阈值未达），与延迟无关；
    - **drop (latency > 35 ms)**：Phase 1-B 或 1-C Orin 端到端延迟 > 35 ms 且 INT8 QAT 也无法压回 35 ms 以内 —— TSM 在本硬件 / Orin 预算下不可行；
...
- 转移矩阵 $A_{ij} = P(y_t = j \mid y_{t-1} = i)$（C×C 个参数；C=7 时 49 个，C=14 时 196 个）。从主线训练数据 + ByteTrack 多数投票轨迹离线统计 transition counts，使用 **Laplace 平滑**（pre-committed α，写入 `configs/temporal_hmm.yaml`；典型 α = 1.0，C=14 时相当于 1/(196+196)≈0.5% 先验，足以避免零概率，又不淹没观测）。**非法转移（如 `green → red` 无黄过渡）**的处理由 `temporal_hmm.yaml` 的 `illegal_transition_policy` 字段决定：`hard_zero`（强制 0，依赖标注 / 政策定义的非法集）或 `downweight`（保留 Laplace α 平滑，让数据驱动）。本计划默认 `hard_zero`（与 §0.2 第 3 行"消除非法状态转移"目标一致）。
...
| `configs/temporal_hmm.yaml` | HMM Laplace α + illegal_transition_set + illegal_transition_policy；transition_matrix_path（离线统计输出）；§2.2.1 引用 |
