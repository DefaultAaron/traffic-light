...
- **不能从已训权重 fine-tune（部署候选默认）**：shift 改变了 channel 语义；R1 / R2 baseline 权重在 shift 后的 channel 上没学过对应模式 → **必须从头重训**（脚手架 `runners/__init__.py` 在 `--pretrained-init` 与 DEIM `HGNetv2.pretrained` 上声明契约要求；当前 v1.5 runner 仍为 `NotImplementedError`，Phase 1-A 实现时必须将契约落成运行时校验。详见下文 §1.4.1）。

  **可选诊断 cell（2026-05-09 conflictor iter-1 amendment）**：v1.1 设计仅 shift 1/8 + zero 1/8 channels，理论上保留了大部分预训练 channel 语义。Phase 1-A 调度时**可选**跑一个 pretrained-init 诊断 cell 作为 cost probe（**不可作部署候选**），用于量化 scratch-only 硬约束的真实代价；若 cost probe 显示 pretrained-init 收敛 wall-clock < scratch × 0.5 且精度 delta < −0.5 pp，留作 R3 重评；scratch 仍是默认 deploy 契约；切换 deploy 契约需 B2 + C3 重新审查并 LANDED 在 README 中。
...
**诊断要求（与 activation tags 对齐 — 2026-05-09 添加，2026-05-09 conflictor iter-1 amendment）**：若 `runs/_tsm_activation.json` 的 `approved_failure_mode_tags` 包含 `far_distance_miss` / `occluded_miss` / `motion_blur` 中任一，Phase 1-A 报告必须列出对应 bucket recall delta。Gate-1A 仍按 OR 执行（小目标 recall +2 pp OR 总体 mAP +1 pp）以决定**是否进入 Phase 1-B**，但**进入 deploy 路径**有更严约束：
- **任一激活 tag 对应 bucket 必须有正向改善**（recall delta > 0），否则即使 OR 条件成立，Phase 1-A 只能作为 **diagnostic continuation** 进 Phase 1-B（须在报告中明示 flag："activated tag bucket no improvement"），**不进 deploy 路径**；
- 已激活但未改善的 bucket 同时是 §0.2 重新评估问题归因的强信号，**Phase 1-A 报告必须显式说明保留 vs 回 §0.2 的判断依据**（不接受 silent continuation）。

§1.5.4 TSM 生命周期 rollup d 阶段 deploy 分支已嵌入此约束（"Phase 1-A 通过 Gate-1A AND 激活 tag 对应 bucket recall delta > 0"）。
...
#### 1.5.4 TSM 生命周期 rollup（2026-05-09 添加；与附加组件计划五步约定对齐）
...
- [ ] **c. 消融** — Phase 1-A 概念验证 → Gate-1A（小目标 recall +2 pp OR 总体 mAP +1 pp，OR 条件）+ §1.5 诊断要求 + **激活 tag 一致性**（详见 §1.5 诊断要求）；Phase 1-B 全量训练 → Gate-1B（小目标桶 recall ≥ 0.6 AND 整机延迟 < 26 ms，AND 条件）；Phase 1-C 导出 → 引擎 sidecar 四字段 carry-forward + Orin 实测延迟。
- [ ] **d. 决策** — 写入 `runs/_tsm_decisions.json`（**plural**，与 `runs/_kd_decisions.json` / `runs/_r2_precision_decisions.json` 命名一致）；schema `scripts/_tsm_decision_schema.json`（NEW，deferred；**必须含字段** `pretrained_init_mode {scratch, pretrained_diagnostic}` 与 `deploy_eligible {true, false}`，cost-probe 强制 `deploy_eligible=false`）；executor `scripts/_tsm_decide_phase.py`（NEW，deferred；hard-fail if `pretrained_init_mode != "scratch" AND deploy_eligible == true`，除非 README LANDED 的 deploy-contract revision 明确放行）。决策分支按以下**完整覆盖**（每条引用 Phase 1-A 通过条件均默认包含 §1.5 诊断要求 = 激活 tag 对应 bucket recall delta > 0；diagnostic-only continuation 仅作研究记录，不进任何 defer / deploy 分支）：
    - **deploy**：Phase 1-A 通过 Gate-1A **AND** 激活 tag 对应 bucket recall delta > 0（即 deploy-eligible Phase 1-A 通过）+ Phase 1-B 通过 Gate-1B + Phase 1-C ONNX/TRT 验证通过（含引擎 sidecar 四字段一致 + Orin 实测延迟 < 26 ms）；
    - **defer (INT8 重评)**：deploy-eligible Phase 1-A 通过 + Phase 1-B 或 Phase 1-C Orin 端到端延迟落在 26-35 ms 区间 + Phase 1-B 精度门通过（小目标桶 recall ≥ 0.6）—— 触发 [`additional_components_plan.md`](additional_components_plan.md) §10.6 TSM 探针（同 round 启动 INT8 校准 + export 改造），随后 Phase 1-C 在 INT8 引擎上重测；
    - **defer (Phase 1-C 失败但 1-A/1-B 通过)**：deploy-eligible 1-A + 1-B 通过，但 Phase 1-C ONNX export 检测到自定义算子 / trtexec build 失败 / sidecar 字段缺失 / sidecar engine_sha256 与计算值不一致 / 引擎 eval-parity > 0.01 pp —— defer 至 export 改造完成（属于已识别 carry-forward；**不**触发 INT8）；
    - **drop (activation-bucket mismatch)**：Phase 1-A OR 通过 + 激活 tag 对应 bucket recall delta ≤ 0（即 diagnostic-only path）—— `runs/_tsm_decisions.json` **立即写入 drop**（在 Phase 1-A 报告完成时），Phase 1-B 如继续运行**仅作为 post-drop diagnostic artifact**（写入独立 `runs/<phase_id>/diagnostic_only/`），不得改写该 drop 决策、不得进入 deploy / INT8 defer / Phase 1-C 任何路径；后续按 §0.2 重新评估问题归因。
    - **drop (Gate-1A 失败)**：Phase 1-A 失败（小目标 recall +< 2 pp 且 mAP +< 1 pp）—— §1.5 明确要求**回到 §0.2 重新评估问题归因，禁止 hyperparam iteration**；
    - **drop (Gate-1B 精度失败)**：deploy-eligible 1-A 通过但 Phase 1-B 小目标桶 recall < 0.6（即 §0.2 启动阈值未达），与延迟无关；
    - **drop (latency > 35 ms)**：deploy-eligible 1-A + 1-B 精度通过，Phase 1-B 或 1-C Orin 端到端延迟 > 35 ms 且 INT8 QAT 也无法压回 35 ms 以内（**OR** INT8 QAT 后引入精度退化超 §10.7 drop 门）—— TSM 在本硬件 / Orin 预算下不可行；
    - **drop (导出层不可救)**：Phase 1-C ONNX 导出引入自定义算子且无 Slice+Concat 等价表达（与 §1.5 Phase 1-C "无需自定义算子" 假设违反）—— 实施层失败，drop 重回 §0.2。
...
- 转移矩阵 $A_{ij} = P(y_t = j \mid y_{t-1} = i)$（C×C 个参数；C=7 时 49 个，C=14 时 196 个）。从主线训练数据 + ByteTrack 多数投票轨迹离线统计 transition counts，使用 **Laplace 平滑**（pre-committed α，写入 `configs/temporal_hmm.yaml`）。**默认 `α = 0.1`**（2026-05-09 conflictor iter-2 amendment：α=1.0 在 C=14 / 196 cells 下注入 196 pseudo-counts，对稀疏 TL 转移数据等同 50% 先验，会淹没真实转移结构；α=0.1 注入 ~20 pseudo-counts，约为典型 row 经验观测的 ≤ 10%）。c 阶段消融必须报告 **α ∈ {0.01, 0.1, 1.0}** 三档敏感性扫描；deploy 决策须基于敏感性扫描中表现稳定的 α 值。**非法转移（如 `green → red` 无黄过渡）**的处理由 `temporal_hmm.yaml` 的 `illegal_transition_policy` 字段决定：`hard_zero`（强制 0，依赖标注 / 政策定义的非法集）或 `downweight`（保留 Laplace α 平滑，让数据驱动）。本计划默认 `hard_zero`（与 §0.2 第 3 行"消除非法状态转移"目标一致）。
...
- [ ] **c. 消融** — A/B 对照（Plan A 固定 EMA vs Plan A + HMM）。**评估协议（pre-committed，必须于 c 阶段开始前锁定）**：
    ...
    - **类别跳变率**：`(每条 track 的 argmax-change 计数 / 该 track 的相邻有效帧对数)` 在 eligible tracks 上的均值。
    - **非法转移计数**：在 eligible tracks 上统计违反 `temporal_hmm.yaml` `illegal_transition_set` 的转移总数（绝对计数，不是率）。
    - **baseline**：同 replay 上的 Plan A（固定 EMA）输出。
    - **gate 优先级**：当跳变率与非法转移结论不一致时（如跳变率改善但非法转移未消除），**非法转移 gate 优先**（§0.2 row 3 是触发 HMM 的首要原因；跳变率次要）。
    - 总 mAP / 关键类别 AP 不退化（与 §1.6 同款 caveat：locked input_source / resolution / tracker_mode）；
    - 实测 Orin 延迟（HMM 应 < 0.01 ms，应在测量噪声以内）。
- [ ] **d. 决策** — 三选一写入 `runs/_hmm_decisions.json`（plural；schema TBD，与 `_kd_decisions.json` / `_r2_precision_decisions.json` 字段格式对齐）：
