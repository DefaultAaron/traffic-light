...
- [ ] **d. 决策** — 写入 `runs/_tsm_decisions.json`（**plural**，与 `runs/_kd_decisions.json` / `runs/_r2_precision_decisions.json` 命名一致）；schema `scripts/_tsm_decision_schema.json`（NEW，deferred；**必须含字段** `pretrained_init_mode {scratch, pretrained_diagnostic}` 与 `deploy_eligible {true, false}`，cost-probe 强制 `deploy_eligible=false`）；executor `scripts/_tsm_decide_phase.py`（NEW，deferred；hard-fail if `pretrained_init_mode != "scratch" AND deploy_eligible == true`，除非 README LANDED 的 deploy-contract revision 明确放行）。决策分支按以下**完整覆盖**（每条引用 Phase 1-A 通过条件均默认包含 §1.5 诊断要求 = 激活 tag 对应 bucket recall delta > 0；diagnostic-only continuation 仅作研究记录，不进任何 defer / deploy 分支）：
    ...
    - **drop (activation-bucket mismatch)**：Phase 1-A OR 通过 + 激活 tag 对应 bucket recall delta ≤ 0（即 diagnostic-only path）—— Phase 1-B 仍可作 diagnostic 跑完，但**报告 d-stage 直接判 drop**（不进 deploy / INT8 defer / Phase 1-C 任何路径）；后续按 §0.2 重新评估问题归因；
