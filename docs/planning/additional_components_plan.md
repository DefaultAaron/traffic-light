...
每个组件 §"清单"小节列出五步状态。R2 round 关帧前所有组件须完成 a-c；deploy 候选须完成 d；report e。

---

## 二、推荐开发顺序总表

按 [`../../research/surveys/detection_enhancements.md`](../../research/surveys/detection_enhancements.md) §5.1 优先级（已删除 HDR / SSL / 主动学习）。R2 in-round **必须**完成 §三-§五；§六-§十一 按 5/15 截止节奏 / replay 反馈启动。

| 排名 | 章节 | 组件 | 触发节奏 | 推荐级 | 工作量 |
|---|---|---|---|---|---|
| 1 | §三 | Copy-paste + 类平衡损失 | R2 训练前**必须** | ⭐⭐⭐⭐⭐ | 2-3 天 |
| 2 | §四 | 硬负样本挖掘 | R2 训练前**必须** | ⭐⭐⭐⭐⭐ | 3-5 天 |
| 3 | §五 | 地图先验门控 | 5/15 之前 | ⭐⭐⭐⭐⭐ | ~1 周 |
| 4 | §六 | SAHI 切片推理 | 5/15+ 可接受 | ⭐⭐⭐⭐ | ~1 周 |
| 5 | §七 | 知识蒸馏（KD） | R2 训练同步（"免费午餐"） | ⭐⭐⭐⭐ | 1-2 周 |
| 6 | §八 | 多相机融合 | 需自动驾驶团队对齐 | ⭐⭐⭐⭐ | 取决于现有相机配置 |
| 7 | §九 | 自适应推理 / ROI | 5/15 之前可落地 | ⭐⭐⭐ | ~3 天 |
| 8 | §十 | INT8 QAT | 仅当 SAHI / TSM 需释放预算 | ⭐⭐⭐ | ~1 周 |
| 9 | §十一 | 规划器先验融合 | 与 cross-detection 计划同步 | ⭐⭐⭐ | ~1 周 |
...
- **指标**：稀有类（`full_val_support < 30` 类）AP delta；总 mAP 不退化（参考 §七.6#1 约定）。
- **deploy**：稀有类 AP 平均 +5 pp **或** 总 mAP 不退化 + 至少 1 个稀有类 +5 pp。
- **defer**：稀有类无显著改善（< 2 pp），但总 mAP 不退化 → R3 再试更激进 copy-paste 策略。
- **drop**：总 mAP 退化 > 0.5 pp。
...
- **指标**：在 demo8/11/13 帧上 FP 数；总 mAP 不退化。
- **deploy**：FP 数下降 ≥ 50%（demo8 类背景帧）+ 总 mAP 不退化（−0.2 pp 容忍）。
- **defer**：FP 仅降 20-50%，留作 R3 重评；
- **drop**：召回掉真实灯 ≥ 5%。
...
- **指标**：路口外 FP 数；路口内 recall（按 GPS 标定）。
- **deploy**：路口外 FP −80% 且路口内 recall 不退化（−0.5 pp 容忍）；
- **defer**：路口外 FP −50% 但路口内 recall 退化 0.5-2 pp，留作 R3 重评门限；
- **drop**：路口内 recall 退化 > 2 pp。
...
### 6.1 机制
把 1280×720 输入切成 4 个 640×640（或更多）有重叠 tile，逐 tile 推理，全图 NMS-merge。**不需要重训**，仅推理时启用。
...
- [ ] **a. 脚手架** — [`../../components/knowledge_distillation/`](../../components/knowledge_distillation/__init__.py) **LANDED v1.3，B2 + C3 已 AGREED**（详见 scaffold README）；schema/decision 执行器 `scripts/_kd_decision_schema.json` + `scripts/_kd_decide_cell.py` 仍 deferred
...
- **条件性**：仅当 §六 SAHI 或 TSM Phase 1-C 需要释放预算时启动；
...
- 多任务 / 辅助头（survey §3.6）—— 工作量数量级大，R3 候选；
- TTA（survey §3.8）—— 部署不可行；仅作伪标签生成工具；
- 检测器集成（survey §3.9）—— 部署超预算；离线工具；
- 图像超分预处理（survey §3.11）—— 被 SAHI 主导；
- 合成数据 / CARLA（survey §3.12）—— 优先 R2 实采；
- ISP ROI AE（survey §4.6）—— 仅当 HDR 不可得时的次优。
