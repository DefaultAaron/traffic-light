...
- **defer**：小目标 recall +2-5 pp **AND** FP16 延迟落在 §10.6 INT8 探针 band（50-80 ms）—— 触发 §10.6 INT8 校准，重测 INT8 引擎后回到本规则；
...
- **条件性 + 预探测**：原则上 §十 仅在 §六 SAHI 或 TSM Phase 1-C 需要释放预算时启动。但避免与 §六 形成循环死锁（SAHI 候选 defer 等 INT8；INT8 仅在 SAHI 触发后启动），加入两条预探测路径：
  - **SAHI 探针**：若 §六 inference-only SAHI 实测 FP16 端到端 latency `> 50 ms` 但 `< 80 ms`（即 §6.7 deploy 阈值之外、§6.7 drop 阈值之内），**立即在同 round 启动 §十 INT8 校准与 export 改造**，不等待 §六 决策；§六 在 INT8 引擎上重测决定 deploy / defer / drop。
  - **TSM 探针**：若 [`temporal_optimization_plan.md`](temporal_optimization_plan.md) §1 TSM Phase 1-B 或 Phase 1-C FP16 Orin 端到端延迟落在 `26 ms ≤ latency ≤ 35 ms` 区间，**且** Phase 1-B 精度门 Gate-1B 通过（小目标桶 recall ≥ 0.6 + 激活 tag bucket 改善），**立即在同 round 启动 §十 INT8 校准与 export 改造**；TSM Phase 1-C 在 INT8 引擎上重跑导出 + 验证后回到 temporal §1.5.4 d-stage 决策。precision 门未通过的 1-A diagnostic-only continuation **不**触发 INT8（避免在已知非 deploy 路径上烧 wall-clock）。
