...
| 组件 | v2.0 状态 | 输出 |
|---|---|---|
| §三 Copy-paste + 类平衡 | a + b LANDED (`d2d5fc9`)；c/d 待启动（前置：trainer-time hook + §四 FP manifest）| `runs/_copy_paste_decision.json` |
| §四 硬负样本挖掘 | a + b LANDED (`d2d5fc9`)；c/d 待启动（前置：Track B FP-harvest manifest）| `runs/_hard_negative_decision.json` |
...
- [ ] c. A/B：无硬负 vs 有硬负；记录 FP、真实灯 recall、总 mAP。**c-stage 启动前置（Track B 任务）**：R1 baseline 跑 demo8/11/13 + R2 难场景；构建 `bg/` / empty-image；人工核验 ≥10%；FP-harvest manifest 冻结落盘后 c. 可启动。
