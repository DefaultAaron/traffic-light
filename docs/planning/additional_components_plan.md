...
- **deploy**：稀有类 AP 平均 +5 pp **或** 至少 1 个稀有类 +5 pp **AND** 所有稀有安全类 AP delta ≥ −1 pp（即不为单一类拉升而牺牲其他稀有类）**AND** 稀有类相关 FP 上升 ≤ 10%（防止"飘在天上"样本引入 FP 暴增；**FP 计数使用 §4.7 frozen manifest 的真实灯 + 背景混合集，阈值固定 confidence ≥ 0.25 / NMS IoU = 0.5；评估时不得替换 frame 子集或阈值，与 §4.7 同源**）**AND** 总 mAP 不退化（−0.2 pp 容忍）。
...
- **defer**：路口外 FP −50% **AND** 路口内 recall 退化 0.5-2 pp **AND** 负面控制场景 missed safety-critical = 0，留作 R3 重评门限。
- **drop**：路口内 recall 退化 > 2 pp **OR** 负面控制场景出现 ≥ 1 missed safety-critical light（GPS drift / 临时信号触发 silent recall failure 是 drop 强信号；不接受 silent failure mode）。**deploy 与 defer 都强制 missed safety-critical = 0**——任何 silent miss 直接 drop。
...
### 6.7 决策规则（pre-committed；2026-05-09 conflictor iter-2 amendment：与 §10.6 INT8 探针 latency band 协调）
- **指标**：小目标桶（bbox 高度 < 20 px）recall delta；端到端 Orin 延迟（locked caveat columns 同 R2 精度奇偶 + temporal）。
- **deploy**：小目标 recall +5 pp **AND** 端到端延迟 `< 50 ms`（半开区间，与 §10.6 探针 `50 ms ≤ latency < 80 ms` 边界互补无重叠）；
- **defer**：小目标 recall +2-5 pp **AND** FP16 延迟落在 §10.6 INT8 探针 band（`50 ms ≤ latency < 80 ms`）—— 触发 §10.6 INT8 校准，重测 INT8 引擎后回到本规则；
- **drop**：小目标 recall +2 pp 以下 **OR** FP16 延迟 ≥ 80 ms（INT8 探针 band 之上，加速空间不足）**OR** INT8 重测后端到端延迟 > 60 ms（INT8 后仍超预算，硬件不可行）。
...
- **条件性 + 预探测**：原则上 §十 仅在 §六 SAHI 或 TSM Phase 1-C 需要释放预算时启动。但避免与 §六 形成循环死锁（SAHI 候选 defer 等 INT8；INT8 仅在 SAHI 触发后启动），加入两条预探测路径：
  - **SAHI 探针**：若 §六 inference-only SAHI 实测 FP16 端到端 latency 落在 `50 ms ≤ latency < 80 ms`（**半开区间，与 §6.7 deploy `< 50 ms` / drop `≥ 80 ms before INT8` 边界互补无重叠无空缺**），**立即在同 round 启动 §十 INT8 校准与 export 改造**，不等待 §六 决策；§六 在 INT8 引擎上重测决定 deploy / defer / drop。
  - **TSM 探针**：若 [`temporal_optimization_plan.md`](temporal_optimization_plan.md) §1 TSM Phase 1-B 或 Phase 1-C FP16 Orin 端到端延迟落在 `26 ms ≤ latency ≤ 35 ms` 区间，**且** `runs/_tsm_decisions.json` 预检查同时满足以下三谓词（命名与 temporal §1.5.4 d-stage 一致以避免 prose drift）：
    1. `deploy_eligible == true`（即 Gate-1A 通过 AND 激活 tag 对应 bucket recall delta > 0）；
    2. `pretrained_init_mode == "scratch"`；
    3. **Phase 1-B 精度门通过**——指小目标桶 recall ≥ 0.6 一项**精度**判据；**不**指 Gate-1B 完整复合门（Gate-1B 含 latency < 26 ms，与本探针 26-35 ms 触发条件互斥；若用 Gate-1B 完整门会导致探针不可达）。
    
    满足以上三条则**立即在同 round 启动 §十 INT8 校准与 export 改造**；TSM Phase 1-C 在 INT8 引擎上重跑导出 + 验证后回到 temporal §1.5.4 d-stage 决策。**`pretrained_diagnostic` 或 `deploy_eligible == false` 路径一律不触发 INT8**（已知非 deploy / cost-probe 路径不烧 wall-clock）。
