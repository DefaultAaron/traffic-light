...
| DEIM-D-FINE-L | 0.857 | 0.611 | — | — | — | 102 / 102 (best=72) | DEIM 容量上限 — 同域 val 与 M 持平（-0.2 / -0.2 pp），不进入部署候选；保留为 KD A6 / A7 教师候选 |
...
- **DEIM-S**: 132 / 132 epochs 完整训练（stage_2 = epochs 120–131，含 12 个 no-aug epoch）。**`best_stat` 在最后一个 epoch (131) 仍创新高** → 训练曲线未饱和 — 详见下文 §DEIM 收敛性核查。eval/latest.pth 即对应 best_stg2.pth 快照（最终 epoch 即最佳）。
...
- **DEIM-L**: **102 / 102 epochs 完整训练**（2026-05-12 完成；best epoch=72；指标快照源 `logs/deim-d-fine_l.log` epoch-72 COCO 评估块 + `runs/detect/deim_dfine_l-r1/eval/best_coco_summary.json`）。**最终聚合指标** AP@0.5=0.857 / AP@0.5:0.95=0.611 / AP@0.75=0.694 / AP_small=0.526 / AR@100=0.726。
  ...
  - **carry-forward**：per-class AP 仍待补；DEIM 训练流的 print loop 仅输出 COCO 12-tuple 聚合值，per-class 需从远端 `eval/latest.pth`（faster_coco_eval 序列化）解析或重跑 `test-only`。见 §R1→R2 桥接（消化项 C）。
...
|------|-----------|-----------|----------|-----------|--------|-------------|---------|----------|-----------|----------|--------|
| DEIM-S | best_stg2.pth | **131 / 131** | 132 | stop_epoch=120；stage-2=120..131（12 ep no-aug） | 0.848 | 0.602 | 0.664 | 0.517 | 0.757 | 0.774 | 0.736 |
| DEIM-M | best_stg2.pth | **97 / 101**  | 102 | stop_epoch=90；stage-2=90..101 | 0.859 | 0.613 | 0.640 | 0.530 | 0.770 | 0.783 | — |
...
| DEIM-L COCO eval 快照 | `runs/detect/deim_dfine_l-r1/eval/best_coco_summary.{json,txt}` | epoch-72 COCO 12-tuple 聚合 + 说明；per-class 待补 |
...
| — | **carry-forward**：DEIM-L per-class AP 提取（消化项 C，见 §R1→R2 桥接）|
...
| **C** | DEIM-L per-class AP 提取 | DEIM-L `best_stg1.pth` + 远端 `eval/latest.pth` 同步 | `runs/detect/deim_dfine_l-r1/eval/per_class_AP.json` + 表格行补回 §专项评估 sister table | 一次性脚本 + 训练机器 GPU | 远端 `eval/latest.pth` 同步完成 | 参考 `runs/detect/deim_dfine_{s,m}-r1/eval/latest.pth` 解析路径 |
...
- **等待权重同步**：B, C — 仅 DEIM-L 远端 → 本地权重同步完成后启动。
...
- 消化项 **A / B / C** → 进入 R2 §七 KD cell matrix 当作 R1-data 预演证据，**不**作为 R2 KD 实测结果。
