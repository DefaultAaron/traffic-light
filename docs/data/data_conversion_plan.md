...
- **按 7 类（R1）进行分层划分**：7 类是 3 类的细分子集，按 7 类分层后 3 类自动平衡（反之不成立）
- **当前状态（2026-04-23）**：R1 7 类分层已完成，`data/merged/` 即为 R1 训练就绪数据集（YOLO 标签 + COCO JSON 双出）。R2 类别锁定后需重跑合并脚本按新 `nc` 分层。
- **训练/验证集划分在两阶段间保持一致**：同一张图片在各阶段中始终属于同一集合（训练或验证），确保评估公平性
